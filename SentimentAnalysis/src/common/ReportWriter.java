...
public class ReportWriter {
	...
	public static void writeHTMPage(PrintWriter out) {
		...
		for(int i=0;i<users.size();++i)
			out.println(users.get(i).partName+"<br>");
		out.println("<br><br><br><h2>Список сообщений</h2><br>");
		for(int i=0;i<messages.size();++i) {
			out.println("<br>Отправитель: <b>"+messages.get(i).getNodeFrom().partName+"</b>");
			out.println("<br>Получатель: <b>"+messages.get(i).getNodeTo().partName+"</b>");
	...
	private static void writeScheme(PrintWriter out) {
		...
		img.getGraphics().setColor(EntityEditor_Helper.colorBackground);
	...
	public static Rectangle cropImage(BufferedImage img) {
		...
		int colorBG=EntityEditor_Helper.colorBackground.getRGB(),colorUp=0,colorDown=0;
