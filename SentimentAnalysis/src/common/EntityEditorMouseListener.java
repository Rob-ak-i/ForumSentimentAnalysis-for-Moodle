...
import gui.GUISchematicEditorContextMenu;
...
public class EntityEditorMouseListener extends MouseInputAdapter{
	...
	*/
    ...
    private void openContextMenu(MouseEvent e) {
    	...
    	contextMenu.setBounds(xScreen, yScreen, 50, 150);
    	contextMenu.compileMenu();
    	contextMenu.setVisible(true);
    }
    public void mouseReleased(MouseEvent e) {
    	if(e.getButton()==3) {openContextMenu(e);return;} else {CommonData.frame.contextMenu.setVisible(false);}
		int xCanvas=e.getX();//cur.x;
		int yCanvas=e.getY();//cur.y;
		xPop=xCanvas;
		yPop=yCanvas;
		Dot  MousePosOnRealMap= EntityRenderer.getRealDotFromImageCoords(xCanvas, yCanvas);
		...
		case MODE_MOVE:
			if(CommonData.partsSelected.size()!=1) {
				...
				for(int i=0;i<CommonData.partsSelected.size();++i) {
					...
					centerX+=part.getPosX();
					centerY+=part.getPosY();
				}
				...
				for(int i=0;i<CommonData.partsSelected.size();++i) {
					part = CommonData.partsSelected.get(i);
					newX=part.getPosX()+deltaX;
					newY=part.getPosY()+deltaY;
		...
		case MODE_SELECT:
			...
			if(tempelement.selected==1) {
				if(CommonData.partsSelected.size()>0) {
					int index=CommonData.partsSelected.indexOf(tempelement);
					if(index>-1) {
						CommonData.partsSelected.remove(index);
						tempelement.selected=0;
					}
			...
			if(CommonData.partsSelected.size()==0) {
				CommonData.nameedit.setText(tempelement.getName());
				CommonData.captionedit.setText(tempelement.getProperties());
			}else {
				CommonData.nameedit.setText(common.Lang.InnerTable.Item.itemNameEditFantomMultipleCaptionName);
				CommonData.captionedit.setText(common.Lang.InnerTable.Item.itemCaptionEditFantomMultipleCaptionName);
			}
